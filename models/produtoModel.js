...
class ProdutoModel {
    ...
    get produtoValorUnitario() { return this.#produtoValorUnitario; } set produtoValorUnitario(produtoValorUnitario) {this.#produtoValorUnitario = produtoValorUnitario;}
    get produtoImagem() { return this.#produtoImagem; } set produtoImagem(produtoImagem) {this.#produtoImagem = produtoImagem;}


    constructor(produtoId, produtoCodigo, produtoNome, produtoQuantidade, categoriaId, marcaId, categoriaNome, marcaNome, produtoValorUnitario, produtoImagem) {
    ...
    async gravar() {
        if(this.#produtoId == 0){
            ...
            let valores = [this.#produtoCodigo, this.#produtoNome, this.#produtoQuantidade, this.#categoriaId, this.#marcaId, this.#produtoValorUnitario, this.#produtoImagem];

            return await conexao.ExecutaComandoNonQuery(sql, valores);
        }
        else{
            //alterar
            let sql = "update tb_produto set prd_cod = ?, prd_nome =?, prd_quantidade= ?, cat_id = ?, mar_id = ?, prd_valorunitario = ?, prd_imagem = ? where prd_id = ?";

            let valores = [this.#produtoCodigo, this.#produtoNome, 
                this.#produtoQuantidade, 
                this.#categoriaId, 
                this.#marcaId, this.#produtoValorUnitario, this.#produtoImagem, this.#produtoId];
    ...
    async buscarProduto(id){
        ...
        if(rows.length > 0){
            ...
            && fs.existsSync(global.CAMINHO_IMG_REAL + global.CAMINHO_IMG_NAV + row["prd_imagem"])) {
            ...
            row['cat_id'], row['mar_id'], row["cat_nome"], row["mar_nome"], row["prd_valorunitario"], imagem);
